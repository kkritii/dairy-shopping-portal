<?php
    ...
    if(isset($_POST['product_submit'])){
        $product = array($_POST['product_name'],$_POST['category'],$_POST['price'],$_POST['size'],$_POST['description']);
        if(empty($product[0]) || empty($product[1]) || empty($product[2]) || empty($product[3])){
            ...
            {       
                if(mysqli_num_rows($result) >= 1){
                    ...
                    $sql = "INSERT INTO product (`product_name`,`category`,`unit_price`,`size`,`description`) VALUES ('$product[0]','$product[1]','$product[2]','$product[3]','$product[4]')";
